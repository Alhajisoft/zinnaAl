      ...
      <div className="container mx-auto px-4 md:px-6 max-w-7xl space-y-12">
        ...
        <div className="flex flex-col lg:flex-row items-start  gap-8">
          ...
          <div className="flex-1 rounded-2xl overflow-hidden h-[320px] mt-20">
            <div className={`${problemCards[0].bgColor} p-6 relative h-full`}>
              {/* Badge */}
              <div className={`inline-flex items-center ${problemCards[0].badgeColor} px-4 py-1 rounded-full text-sm font-bold mb-4`}>
                <span className="w-3 h-3 bg-white rounded-full mr-2"></span>
                {problemCards[0].badge}
              </div>

              {/* Title */}
              <h3 className={`text-[32px] font-bold ${problemCards[0].textColor} leading-tight mb-2`}>
                Unfinished
                <br /> video-based
                <br /> Courses
              </h3>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 rounded-2xl overflow-hidden h-[320px] ">
            <div className={`${problemCards[1].bgColor} p-6 relative h-full flex gap-3 `}>
                <div className='w-full h-full'>
                <div className={`inline-flex items-center ${problemCards[1].badgeColor} px-4 py-1 rounded-full text-sm font-bold mb-4`}>
                <span className="w-3 h-3 bg-white rounded-full mr-2"></span>
                {problemCards[1].badge}
              </div>
              <h3 className={`text-[28px] font-bold ${problemCards[1].textColor} mb-2`}>
                {problemCards[1].title}
              </h3>
              <p className={`text-sm ${problemCards[1].textColor}`}>
                {problemCards[1].description}
              </p>
              <img
                src={problemCards[1].imageUrl}
                alt="using the app"
                className="absolute right-4 top-8 w-32 h-auto object-cover rounded-xl bg-amber-400"
                onError={(e) => {
                  e.currentTarget.src =
                    'https://via.placeholder.com/150x300/666/fff?text=Burnout';
                }}
          ...
          {/* Card 3 */}
          <div className="flex-1 rounded-2xl overflow-hidden h-[320px]">
            <div className={`${problemCards[2].bgColor} p-6 relative h-full flex gap-3`}>
            <div className='w-full h-full'>
                  <div className={`inline-flex items-center ${problemCards[2].badgeColor} px-4 py-1 rounded-full text-sm font-bold mb-4`}>
                <span className="w-3 h-3 bg-white rounded-full mr-2"></span>
                {problemCards[2].badge}
              </div>
              <h3 className={`text-[28px] font-bold ${problemCards[2].textColor} mb-2`}>
                No feedback on
                <br /> your learning
              </h3>
            ...
            <div className='h-full w-full cursor-pointer ' role='button'  onClick={openModal}>
<img src={problemCards[2].imageUrl} alt="Using the app" className="w-full h-full object-cover rounded-lg" />
