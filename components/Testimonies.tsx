...
const Avatar: React.FC<AvatarProps> = ({ src, alt, bgColor }) => {
    ...
    return (
        <div className={`w-12 h-12 ${colorVariants[bgColor]} rounded-full flex items-center justify-center border-2 transition-all duration-300 group-hover:scale-110 group-hover:shadow-md`}>
            <div className={`w-8 h-8 ${innerColorVariants[bgColor]} rounded-full flex items-center justify-center overflow-hidden transition-transform duration-300 group-hover:scale-105`}>
                <img
                    src={src}
                    alt={alt}
