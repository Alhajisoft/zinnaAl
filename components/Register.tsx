...
import { Check, ArrowLeft, User, Mail, Phone, MapPin, MessageCircle, CreditCard, Calendar, Clock } from 'lucide-react';
...
const Course: React.FC = () => {
    const [currentPage, setCurrentPage] = useState<'courses' | 'registration' | 'payment'>('courses');
    ...
    const validateForm = () => {
        const { studentName, email, phoneNumber, address, whatsappNumber } = registrationData;
        ...
        try {
            ...
            const handler = paystack.setup({
                ...
                callback: function(response: any) {
                    alert('Payment successful! Registration completed. Transaction reference: ' + response.reference);
    ...
    if (currentPage === 'registration') {
        return (
            <>
                ...
                <div className="min-h-screen bg-gradient-to-br from-gray-50 to-amber-50 py-8 md:py-16 px-4 sm:px-6 lg:px-8">
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="mb-8">
                            ...
                            <div className="bg-gradient-to-r from-[#1D1001] to-amber-800 rounded-2xl p-6 text-white shadow-xl">
                                <h1 className="text-3xl sm:text-4xl font-bold mb-4">Course Registration</h1>
                                {selectedCourse && (
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white/10 rounded-xl p-4">
                                        ...
                                        <div className="text-right">
                                            <div className="text-2xl font-bold">{formatPrice(selectedCourse.price)}</div>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-gray-100">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-2xl font-bold text-[#1D1001]">Student Information</h2>
                            ...
                            <div className="mt-8">
                                <button
                                    ...
                                    className={`w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all duration-300 transform hover:scale-105 focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 ${
                                        validateForm()
                                            ? 'bg-gradient-to-r from-[#1D1001] to-amber-800 text-white hover:from-amber-800 hover:to-[#1D1001] shadow-lg'
    ...
    if (currentPage === 'payment') {
        return (
            <div className="min-h-screen bg-gradient-to-br from-[#1D1001] to-amber-900 py-8 md:py-16 px-4 sm:px-6 lg:px-8">
                <div className="max-w-2xl mx-auto">
                    {/* Header */}
                    <div className="mb-8">
                        <button
                            onClick={() => setCurrentPage('registration')}
                            className="flex items-center text-white hover:text-amber-200 transition-colors mb-6 group"
                        >
                            <ArrowLeft className="w-5 h-5 mr-2 group-hover:-translate-x-1 transition-transform" />
                        ...
                        <div className="text-center text-white mb-8">
                            ...
                            <p className="text-amber-200">Secure payment powered by Paystack</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8">
                        {selectedCourse && (
                            <>
                                {/* Course Summary */}
                                <div className="bg-gradient-to-r from-[#1D1001] to-amber-800 text-white p-6 rounded-xl mb-6">
                                    ...
                                    <div className="flex justify-between items-center text-sm opacity-90 mb-4">
                                        ...
                                        <span>{selectedCourse.duration} {selectedCourse.durationType}</span>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-3xl font-bold">{formatPrice(selectedCourse.price)}</div>
                                    </div>
                                </div>

                                {/* Student Details */}
                                <div className="bg-gray-50 p-6 rounded-xl mb-6">
                                    <h4 className="font-semibold text-[#1D1001] mb-4">Student Details</h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <span className="font-medium text-gray-600">Name:</span>
                                            <div className="text-[#1D1001]">{registrationData.studentName}</div>
                                        </div>
                                        <div>
                                            <span className="font-medium text-gray-600">Email:</span>
                                            <div className="text-[#1D1001]">{registrationData.email}</div>
                                        </div>
                                        <div>
                                            <span className="font-medium text-gray-600">Phone:</span>
                                            <div className="text-[#1D1001]">{registrationData.phoneNumber}</div>
                                        </div>
                                        <div>
                                            <span className="font-medium text-gray-600">WhatsApp:</span>
                                            <div className="text-[#1D1001]">{registrationData.whatsappNumber}</div>
                                        </div>
                                        <div className="sm:col-span-2">
                                            <span className="font-medium text-gray-600">Address:</span>
                                            <div className="text-[#1D1001]">{registrationData.address}</div>
                                        </div>
                                    </div>
                                </div>
